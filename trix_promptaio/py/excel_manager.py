...
class ExcelStyleManager:
                    ...
                    if sheet_path in z.namelist():
                        try:
                            ...
                            for row in rows_list:
                                ...
                                sc = slug_counts.get(slug, 0) + 1
                                slug_counts[slug] = sc
                                style_id = f"{cat_name}_{slug}" if sc == 1 else f"{cat_name}_{slug}_{sc}"
                                ...
                                if 1 in r_imgs and 2 in r_imgs:
                                    ...
                                    fn1 = f"{style_id}_1{ext1}"
                                    fn2 = f"{style_id}_2{ext2}"
        ...
        for path in excel_files:
            ...
            if modified:
                ...
                try:
                    ...
                    logger.error(f"Failed to write auto-tags to {cat_clean}.xlsx: {e}")
    ...
    def export_category(cls, category_name: Optional[str] = None) -> Tuple[bytes, str, str]:
        """
        Exports a specific category .xlsx file or all categories as a Zip archive.
        ...
        # Export ALL as a ZIP bundle of category .xlsx files
        excel_files = cls.get_excel_files()
        zip_buf = io.BytesIO()
    ...
    def import_excel_file(cls, file_bytes: bytes, filename: str) -> dict:
        ...
        # Build existing tags set from other files
        ...
        # The auto-tagging and rewriting is now handled centrally in load_master_data
        cls.clear_cache()
