...
Пакет включает две основные ноды управления: `🌊Bypass Nodes w Groups by ID` (с группировкой по категориям и сворачиваемыми списками) и `🌊Bypass Nodes by ID` (упрощенный линейный список).
...
### Вариант 1: Через ComfyUI Manager (Рекомендуемый)
1. Откройте **ComfyUI Manager** в интерфейсе ComfyUI.
2. Нажмите кнопку **Custom Nodes Manager**.
3. Введите в поиске `ComfyUI-TrixNodes` (или введите имя издателя `trix7111`).
4. Нажмите кнопку **Install** рядом с найденной нодой.
